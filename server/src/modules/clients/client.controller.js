...
export const clientController = {
  ...
  try {
    console.log('🔍 findOne çağrıldı, ID:', req.params.id); // ← BUNU EKLE!
    const client = await clientService.findOne(req.params.id);
    console.log('📋 Bulunan client:', client); // ← BUNU EKLE!
    return successResponse(res, client, 'Client fetched successfully');
  } catch (error) {
    console.error('❌ Hata:', error); // ← BUNU EKLE!
  ...
  async update(req, res) {
    try {
      ...
      await AuditLog.create({
        ...
        entity_id: client.id,
        user_id: req.user.id,
        description: `Client ${client.first_name} ${client.last_name} updated`,
        ip_address: req.ip,
        user_agent: req.headers['user-agent'],
      });
  ...
  async remove(req, res) {
    try {
      ...
      await AuditLog.create({
        ...
        user_id: req.user.id,
        description: `Client ${client.first_name} ${client.last_name} deleted`,
        ip_address: req.ip,
        user_agent: req.headers['user-agent'],
      });
